...
def execute_job(
    ...
    after ``next_attempt_at_jst``. ``retry_sleep`` is kept only for backward
    compatibility; retries are never slept inline.
    """
...
def _fail(
    ...
    next_attempt = fmt_jst(now_jst() + timedelta(seconds=backoff))
    db.set_job_status(conn, job_id, "failed", error, next_attempt)
    conn.commit()
    log.warning("job %s attempt %d failed (%s); retry after %s",
                job_id, attempt_no, error, next_attempt)
    return "retry"
...
def collect_due_once(
    ...
    """Execute every currently-due job once without blocking on retry backoff."""
    own = client is None
    client = client or PoliteClient()
    stats: dict[str, int] = {}
    try:
        recovered = db.recover_interrupted_jobs(conn)
        if recovered:
            conn.commit()
        ...
        for job in jobs:
            try:
                status = execute_job(conn, job, client, retry_sleep=False)
            except DayAborted:
