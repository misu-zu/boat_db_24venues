...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    _migrate_capture_jobs(conn)
    conn.commit()


def _migrate_capture_jobs(conn: sqlite3.Connection) -> None:
    columns = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(capture_jobs)").fetchall()
    }
    if "next_attempt_at_jst" not in columns:
        conn.execute("ALTER TABLE capture_jobs ADD COLUMN next_attempt_at_jst TEXT")
...
def set_job_status(
    ...
    next_attempt_at_jst: str | None = None,
) -> None:
    conn.execute(
        "UPDATE capture_jobs SET status=?, last_error=?,"
        " next_attempt_at_jst=?, updated_at_jst=?"
        " WHERE job_id=?",
        (status, last_error, next_attempt_at_jst, fmt_jst(now_jst()), job_id),
    )
...
def recover_interrupted_jobs(conn: sqlite3.Connection) -> int:
    """Return orphaned running jobs to the queue after a process restart."""
    cur = conn.execute(
        "UPDATE capture_jobs"
        " SET status='pending',"
        "     attempt_count=CASE"
        ...
        "         ELSE 0"
        "     END,"
        "     last_error='recovered from interrupted run',"
        "     next_attempt_at_jst=NULL,"
        "     updated_at_jst=?"
...
def due_jobs(conn: sqlite3.Connection, now_iso: str) -> list[sqlite3.Row]:
    return conn.execute(
        ...
        "   AND (j.next_attempt_at_jst IS NULL OR j.next_attempt_at_jst <= ?)"
        " ORDER BY j.scheduled_at_jst",
        (config.MAX_RETRIES, now_iso, now_iso),
    ).fetchall()
