...
| リクエスト間隔 | 3秒以上 |
| タイムアウト | 接続5秒 / 読込15秒 |
| 再試行 | 最大3回(次回試行時刻をDBに記録: 15s → 45s → 120s) |
| HTTP 403/429 | その日の自動収集を停止(CRITICALログ) |
| User-Agent | 明示(連絡先付き) |

アクセス制限を回避する処理は実装していない。
失敗ジョブの待機は `time.sleep` で daemon 全体を止めず、他場・他スロットのdueジョブを先に処理する。
...
同じ出力先への再実行は既存 dataset を置換するため、既存パーティションへの追記重複は発生しない。
...
37テストで、120件抽出・final判別・no_data判別・119件拒否・冪等性・
